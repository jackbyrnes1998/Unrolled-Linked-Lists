...
// ===================================================================
void MoreTests() {
    UnrolledLL<std::string> a;

    // A bunch of push_back and push_front that will overflow the node
    a.push_back("OHMYGOD");
    a.push_front("DAYUM");
    a.push_back("JESUS");
    a.push_back("CRIST");
    a.push_front("HOWTOSPELL");
    a.push_front("CRIST");
    a.push_back("GIRLS");
    a.push_front("IN");
    a.pop_front();
    ...
    //erase that erase the whole node
    UnrolledLL<std::string>::iterator a_iter = a.begin();
    a_iter = a.begin();
    while (a_iter != a.end()) {
        if (*a_iter == "GUYS") {
            a_iter = a.erase(a_iter);
    ...
    // insert that overflow a node. It will create a new node
    for (UnrolledLL<std::string>::iterator itr = a.begin(); itr != a.end(); itr++) {
        if (*itr == "CRIST") {
            itr = a.insert(itr, "RPIGIRLSBICHY");
            itr++;
        }
    }
